SMCEM_Uonestep=function(Y,mu,Sigma,df,nob,e){
  ...
  for (i in 1:n){
    if (is.element(i, W))
    {
      ...
      Y1[i,bb]=colMeans(rmvt(n=nob,delta=K1$condMean,sigma=K1$condVar,df=K1$cond_df))

      M=mahalanobis(Y1[i,-bb],mu[-bb],solve(K1$phi))
      Weight[i]=mean(rgamma(100,(df+pp[i])/2,(df+M)/2))
    ...
    #Weight[i]=(df+p)/(df+M)
    Weight[i]=mean(rgamma(100,(df+p)/2,(df+M)/2))
    MPHI=matrix(rep(0,p*p),p)
